...
* `dataset` is required: it names the dataset the server serves, giving `{url}:{port}/{dataset}/sparql`, `/update` and `/data`.
* Authentication is optional. A stock Fuseki serves its dataset endpoints unauthenticated, so `username`/`password` are sent only when configured (in the `_secrets` file).
* `inputGraph` is optional, and affects *reads* only. An unqualified INSERT or DELETE writes to the dataset's default graph (SPARQL 1.1 Update §3.1.3) and Fuseki has no non-standard parameter to redirect it — unlike GraphDB's `insert-graph-uri`. So either leave `inputGraph` unset, and the `given` goes to the default graph where unqualified updates behave as written, or name the graph in the query with `GRAPH`/`WITH`.
...
Fuseki is the backend mustrd exercises for real. `scripts/fuseki_integration.sh` starts one in Docker and runs the *same* `expected-success` specs the RDFLib suite runs, and CI does the same on every push. The other HTTP backends are mock-tested, because each needs a licence or a server; what Fuseki covers is everything they share — request building, Accept headers, dataset parameters, and the `given`-upload round trip.

==== For Stardog:

* `database` is required to identify the Stardog database to run against.
* `inputGraph` is required: it is the materialised named graph the `given` data is loaded into.
* Authentication prefers a bearer `token`. If no token is given, `username`/`password` basic auth is used instead (store these in the `_secrets` file).
* `materialisedGraph` and `virtualGraph` are optional and repeatable. Each names an additional graph to include in the dataset a query runs over, so the *same* query can be tested against a chosen combination of materialised (physically stored) and virtual (data-virtualisation) named graphs. To compare combinations, define several Stardog configs that differ only in which graphs they include, and run the same spec against each.
...
:stardog_materialised_and_virtual a :Stardog ;
    :url "http://localhost" ;
    :port "5820" ;
    :database "mustrd" ;
    :token "your-bearer-token" ;
    :inputGraph "http://mustrd.org/test_graph" ;
    :materialisedGraph "http://example.org/reference-data" ;
    :virtualGraph "virtual://customers" .
----
...
----
A quad format — TriG (.trig), NQuads (.nq), TriX — keeps its named graphs, so a `when` can read one with a `GRAPH` clause. A `when` with no `GRAPH` clause still sees every graph in the file.
----
    must:given [ a must:FileDataset ;
                 must:file "test/data/given-named-graphs.trig" ] ;
    must:when  [ a must:TextSparqlSource ;
                 must:queryText "SELECT ?s WHERE { GRAPH <https://example.org/graph-a> { ?s ?p ?o } }" ;
...
A SPARQL ASK answers a boolean, so its `then` is a `must:AskResult` carrying the boolean it should answer. There is no table or graph to compare.
...
`must:boolean false` is an assertion in its own right — that the ASK answers false, not that the spec has no expectation. Useful for asserting something is *absent*: that a filter removed what it should, or that no row leaked into a layer.

An ASK reads only. Nothing is written to the `given`, which matters when specs chain.

DESCRIBE is not supported. A spec declaring `must:DescribeSparql` is rejected by SHACL validation, which names the query types that are allowed.
...
* *FileDataset* - The dataset is a local file containing serialised RDF or tabular data. The formats supported are the same as those for the RDFLib Graph().parse function i.e. Turtle (.ttl), NTriples (.nt), N3 (.n3), RDF/XML (.xml) and TriX, as well as tabular formats (.csv, .xls, .xlsx). A quad format is accepted here too; by default the quads are read as one flat union — see below.

==== Comparing named graphs in a then

A `then` is compared as one flat union by default. You should not have to say which named graph a triple is in just to assert that it exists, so a quad `then` passes as long as the triples are all present, wherever they sit.

Add `must:matchNamedGraphs true` and the layout becomes part of the assertion: the right triples in the wrong graphs is a failure, and the failure names the graphs that differ.

----
    must:when  [ a must:TextSparqlSource ;
                 must:queryText """INSERT { GRAPH ?g { ?s ex:copied ?o } }
                                   WHERE  { GRAPH ?g { ?s ?p ?o } }""" ;
                 must:queryType must:UpdateSparql ] ;
    must:then  [ a must:FileDataset ;
                 must:matchNamedGraphs true ;
                 must:file "test/data/expected.trig" ] .
----

Worth using when the graph a triple lands in is the thing under test — a pipeline that writes each layer to its own graph, for instance. Leave it off otherwise.

==== Which query forms can produce named graphs

An UPDATE can always write to a named graph — `INSERT { GRAPH ?g { ... } }` is standard SPARQL.

A CONSTRUCT is where it gets engine-specific, and this catches people out. **Standard SPARQL 1.1 cannot construct quads**: the `CONSTRUCT` template is a set of triple patterns, with no `GRAPH` allowed. Putting one there is a syntax error, not an empty result — so on RDFLib you get a parse failure:

----
Expected ConstructQuery, found 'GRAPH'
----

Some engines extend the grammar anyway:

[cols="1,3"]
|===
| Engine | Quad CONSTRUCT

| *RDFLib* (mustrd's default)
| No. `GRAPH` in a construct template is a parse error.

| *Jena / ARQ*
| Yes, an explicit extension since Jena 3.0.1 — `CONSTRUCT { GRAPH :g { ?s :p ?o } } WHERE { ... }`.

| *Stardog*
| Yes, via a graph template — `CONSTRUCT { graph ?g { ?s ?p ?o } } WHERE { ... }`, so the graph can be bound per solution.

| *Anzo / AnzoGraph*
| Yes. Its CONSTRUCT clause takes "a graph and triple template" — 5.3.6 added a *compact* form that lets you omit it — and the Query Builder's *Dry Run* is built on exactly this rewrite.
|===

The reason this is worth knowing is the *dry run*. To preview an `INSERT { GRAPH ?g { ... } } WHERE { ... }` without writing anything, swap `INSERT` for `CONSTRUCT` and keep the template and the `WHERE` exactly as they are. On an engine with the extension you get back the quads the update *would* have written — and with `must:matchNamedGraphs true` on the `then`, mustrd asserts they land in the right graphs.

This is not a trick invented here. Anzo ships it as a button: for an INSERT or DELETE query the Query Builder offers *Dry Run*, which "runs a version of the query where INSERT or DELETE is replaced with CONSTRUCT, and the results report the number of statements that the query affects, i.e., the number of additions or removals **per graph**". Per graph is the tell — the rewritten CONSTRUCT has to be quad-aware to count that way.

What Anzo's button gives you once, interactively, a spec gives you on every run, with the expected graphs written down.

mustrd's Stardog backend asks the server for TriG rather than Turtle precisely so those graph names survive the response; Turtle cannot carry them.

Sources:

* https://www.w3.org/TR/sparql11-query/#construct[SPARQL 1.1 Query, §16.2 CONSTRUCT] — the template is `ConstructTriples`, no `GRAPH`
* https://jena.apache.org/documentation/query/construct-quad.html[Apache Jena ARQ — Construct Quad] — "Jena ARQ extends the grammar of the CONSTRUCT query form"
* https://github.com/w3c-cg/sparql-dev/wiki/Inventory-of-existing-extensions-to-SPARQL-1.1[W3C SPARQL CG — Inventory of existing extensions to SPARQL 1.1] — records both the Jena and Stardog forms
* https://github.com/w3c/sparql-dev/issues/31[w3c/sparql-dev issue 31 — CONSTRUCT GRAPH] — the standardisation request
* https://2024.help.altair.com/5.4/graphstudio/userdoc/query-builder-query.htm[Anzo / Graph Studio — Running SPARQL Queries in the Query Builder] — the *Dry Run* button, and "additions or removals per graph"
* https://2025.help.altair.com/2025.1/graphstudio/userdoc/relnotes/anzo-v53-releases.htm[Anzo 5.3 release notes] — 5.3.6, "a compact version of a CONSTRUCT query that does not require you to provide a graph and triple template in the CONSTRUCT clause"
----
    must:then  [ a must:FileDataset ;
