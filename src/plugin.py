...
def pytest_configure(config) -> None:
    ...
    for test_config_subject in config_graph.subjects(predicate = RDF.type, object = MUST.TestConfig):
        ...
        filter_on_tripleStore = list(config_graph.objects(subject=test_config_subject, predicate= MUST.filterOnTripleStore))
        
        test_configs[test_function] = TestConfig(test_function = test_function, spec_path = spec_path, data_path = data_path,
                                                 triplestore_spec_path = triplestore_spec_path, filter_on_tripleStore = filter_on_tripleStore)
