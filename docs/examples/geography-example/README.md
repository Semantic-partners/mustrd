...
| [`report/term-coverage-example.ttl`](report/term-coverage-example.ttl) | The same result as **RDF** (W3C DQV + PROV) for a knowledge graph — `dqv:QualityMeasurement`s computed on the ontology + `owl:versionIRI`, a `cov:TermCoverage` per term, and quality issues. Generated by `--term-coverage-rdf`. |
