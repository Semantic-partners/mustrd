...
import pathlib
import traceback
from mustrd.mustrdAnzo import create_graphmart_and_layers
...
# Extend pytest to include the --create-graphmart option
def pytest_addoption(parser):
    parser.addoption(
        "--create-graphmart",
        action="store_true",
        default=False,
        help="Enable dynamic graphmart and layer creation for tests",
    )
...
class MustrdTestPlugin:
    ...
    create_graphmart: bool

    def __init__(self, md_path, test_config_file, secrets, create_graphmart):
        self.md_path = md_path
        self.test_config_file = test_config_file
        self.secrets = secrets
        self.create_graphmart = create_graphmart
        self.items = []
    ...
    def pytest_sessionstart(self, session):
        ...
        if self.create_graphmart:
            triple_store = get_triple_stores(self.test_config_file)[0]  # Assuming one triple store for simplicity
            create_graphmart_and_layers(
                triple_store,
                graphmart="DynamicGraphmart",
                input_layer="DynamicInputLayer",
                output_layer="DynamicOutputLayer"
            )
